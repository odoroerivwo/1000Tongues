...
export default function ChoristerPage() {
  ...
  // Update chorister status (approve/reject)
  const updateStatus = async (id: string, status: string) => {
    setActionLoading(id);
    try {
      const res = await fetch(`/api/chorister/${id}/status`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
  ...
  // Filter search using firstName, email, or voicePart
  const filteredChoristers = choristers.filter((ch) =>
    (ch.firstName ?? "").toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  const stats = {
    ...
    pending: choristers.filter((ch) => ch.status === "Pending").length,
  };
  ...
  const exportToExcel = async () => {
    ...
    const worksheet = XLSX.utils.json_to_sheet(
      choristers.map((ch) => ({
        ...
        Email: ch.email ?? "-",
        Phone: ch.phone ?? "-",
        "Voice Part": ch.voicePart ?? "-",
        Status: ch.status,
        Registered: new Date(ch.createdAt).toLocaleDateString(),
      }))
    );
    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          <h3 className="text-sm font-medium text-gray-600">Total Choristers</h3>
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  {["Name", "Email", "Phone", "Voice Part", "Status", "Registered", "Actions"].map(
                    (header) => (
                      <th
                        key={header}
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredChoristers.map((ch) => (
                  <tr key={ch._id} className="hover:bg-gray-50 transition">
                    <td className="px-6 py-4 text-sm font-medium text-gray-900">
                      {ch.firstName ?? "-"}
                    </td>
                    <td className="px-6 py-4 text-sm text-gray-600">{ch.email ?? "-"}</td>
                    <td className="px-6 py-4 text-sm text-gray-600">{ch.phone ?? "-"}</td>
                    <td className="px-6 py-4 text-sm text-gray-600">{ch.voicePart ?? "-"}</td>
                        ...
                        {actionLoading === ch._id ? (
                          ...
                          <>
                            {ch.status === "Pending" && (
                              <>
                                <button
                                  onClick={() => updateStatus(ch._id, "Active")}
                                ...
                                <button
                                  onClick={() => updateStatus(ch._id, "Inactive")}
      ...
      {showModal && (
        <Modal title="Add New Chorister" onClose={() => setShowModal(false)}>
          <ChoristerForm onSubmit={handleSubmit} submitButtonText="Add Chorister" />
        </Modal>
      )}
