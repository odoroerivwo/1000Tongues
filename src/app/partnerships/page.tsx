...
export default function PartnershipsPage() {
  const [partners, setPartners] = useState<Partner[]>([
    { id: 1, name: "Faith Foundation", email: "faith@example.com", phone: "08077778888", organization: "Faith Org", status: "Pending" },
    { id: 2, name: "Praise Union", email: "praise@example.com", phone: "08099990000", organization: "Union Choirs", status: "Accepted" },
  ]);
  ...
  const openEdit = (p: Partner) => { setSelected(p); setModalOpen(true); };
  const saveEdit = () => {
    if (!selected) return;
    setPartners(prev => prev.map(x => (x.id === selected.id ? selected : x)));
    setModalOpen(false);
    setToastMessage("Partner updated successfully!");
  };
  const accept = (id: number) => { setPartners(prev => prev.map(x => (x.id === id ? { ...x, status: "Accepted" } : x))); setToastMessage("Partnership approved!"); };
  const remove = (id: number) => { setPartners(prev => prev.filter(x => x.id !== id)); setToastMessage("Partnership deleted!"); };

  return (
    <AuthGuard>
      <div className="bg-white p-6 shadow rounded-xl relative">
        <h1 className="text-2xl font-bold mb-4 text-slate-800">Partnerships</h1>
        <p className="text-gray-600 mb-6">Manage partnerships. (Mock data — connect to MongoDB when ready.)</p>
            ...
            <tbody className="divide-y divide-gray-100">
              {partners.map(p => (
                <tr key={p.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-3 text-sm text-slate-700">{p.phone}</td>
                  <td className="px-6 py-3 text-sm text-slate-700">{p.organization}</td>
                  <td className="px-6 py-3 text-sm">
                    <span className={`px-3 py-1 rounded-full text-xs font-medium ${p.status === "Accepted" ? "bg-green-100 text-green-700" : "bg-yellow-100 text-yellow-700"}`}>{p.status}</span>
                  </td>
                  <td className="px-6 py-3 text-center flex justify-center gap-3">
                    <button onClick={() => openEdit(p)} className="p-2 text-blue-600 hover:bg-blue-50 rounded-full" title="Edit"><FiEdit /></button>
                    <button onClick={() => accept(p.id)} className="p-2 text-green-600 hover:bg-green-50 rounded-full" title="Accept"><FiCheckCircle /></button>
                    <button onClick={() => remove(p.id)} className="p-2 text-red-600 hover:bg-red-50 rounded-full" title="Delete"><FiTrash2 /></button>
                  </td>
                </tr>
              ))}
            </tbody>
            ...
            <div className="bg-white rounded-lg shadow-xl w-full max-w-md p-6">
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <input value={selected.organization} onChange={(e) => setSelected(prev => prev ? { ...prev, organization: e.target.value } : prev)} className="w-full border rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#0D1B2A]" />
                </div>
