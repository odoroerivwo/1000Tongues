...
interface ChoristerFormProps {
  onSubmit: (data: ChoristerFormData) => Promise<void>;
  initialData?: Partial<ChoristerFormData>;
...
export default function ChoristerForm({
  ...
  const [formData, setFormData] = useState<ChoristerFormData>({
    name: initialData?.name || "",
    email: initialData?.email || "",
    phone: initialData?.phone || "",
    voicePart: initialData?.voicePart || "",
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccess("Chorister registered successfully!");

      // Reset form if it's public registration
      if (isPublic) {
        setFormData({
          name: "",
          email: "",
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-2">
          Voice Part <span className="text-red-500">*</span>
        </label>
        <div className="relative">
          <label htmlFor="voicePart" className="sr-only">
            Select voice part
          </label>
          ...
          >
            <option value="">Select a voice part</option>
            {voiceParts.map((part) => (
